...
export default function App() {
  ...
  const sendWhatsAppOrder = () => {
    ...
    let text = `🔥 *Novo Pedido - Festa Junina 3B* 🔥\n*Senha do Pedido:* ${orderPlaced.id}\n\n*Itens:*\n`;
    orderPlaced.items.forEach(item => {
      const flavorText = item.flavor ? ` - ${item.flavor}` : '';
      text += `${item.quantity}x ${item.menuItem.name}${flavorText} (R$ ${(item.menuItem.price * item.quantity).toFixed(2).replace('.', ',')})\n`;
    });
    text += `\n*Total:* R$ ${orderPlaced.total.toFixed(2).replace('.', ',')}`;
          ...
          <div className="fixed inset-0 z-50 bg-[#3e4f32] flex flex-col items-center justify-center p-6 text-white text-center">
            ...
            >
              ...
              <button
                ...
                className="w-full bg-[#25D366] text-white flex items-center justify-center gap-2 font-bold p-4 rounded-xl uppercase tracking-wider hover:bg-[#1DA851] active:scale-95 transition-all mb-3 shadow-md"
              >
              ...
              <button
                ...
                className="w-full bg-[#d7ceb8] text-[#3e4f32] font-bold p-4 rounded-xl uppercase tracking-wider hover:bg-[#c4b9a1] active:scale-95 transition-all shadow-sm"
              >
                Fazer Novo Pedido
              </button>
            </motion.div>
