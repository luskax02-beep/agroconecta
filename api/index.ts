...
apiApp.post(["/analyzeCrop", "/api/analyzeCrop"], async (req, res) => {
    try {
      ...
      console.error(error);
      const isMissing = error.message === "API_KEY_MISSING";
      const isInvalid = error.message?.includes("API key not valid") || error.status === "INVALID_ARGUMENT" || error.status === 400;
      
      if (isMissing || isInvalid) {
         return res.status(500).json({ error: "Erro de autenticação com a API do Gemini. Certifique-se de que a chave da API é válida." });
      }
...
apiApp.post(["/analyzeTerrain", "/api/analyzeTerrain"], async (req, res) => {
    try {
